...
* Tags: epidemics, covid19, multiplex, multi agent systems
*/
...
//Number of susceptible host at init
	...
	reflex save_agent_attribute{ //when: cycle = 10 {
		ask people {
			...
			save [name, cycle, location, living_place, working_place,worship_place, school, stuff_place_1,stuff_place_2, is_susceptible, is_infected, is_immune] to: "/Volumes/DATA/MAS/Data/Agents/Agents_"+ string(cycle) +".csv" type:"csv" rewrite: false;
...
species building {
	...
	list<list<people>> groups update: group(people_in_building);
	...
	action group (list<people> in_building) {
		loop index from: 0 to: length(people_in_building) - 1 step: 10 {
			if ((index + 10 < length(people_in_building) and (int(index / 10) < length(groups)))) {
				add people_in_building[index] to: groups[int(index / 10)];
				add people_in_building[index + 1] to: groups[int(index / 10)];
				if (flip(0.9)) {
					add people_in_building[index + 2] to: groups[int(index / 10)];
					add people_in_building[index + 3] to: groups[int(index / 10)];
					add people_in_building[index + 4] to: groups[int(index / 10)];
				}

				if (flip(0.5)) {
					add people_in_building[index + 5] to: groups[int(index / 10)];
					add people_in_building[index + 6] to: groups[int(index / 10)];
					add people_in_building[index + 7] to: groups[int(index / 10)];
				}

				if (flip(0.25)) {
					add people_in_building[index + 8] to: groups[int(index / 10)];
					add people_in_building[index + 9] to: groups[int(index / 10)];
				}
	...
	reflex infect when: (n_inf > 0) {
		if (!is_toobig) {
			loop index from: 0 to: length(people_in_building) - 1 step: 1 {
				...
				if ((!agt.is_immune) and agt.is_susceptible and flip(beta * beta_baseline * n_inf / length(people_in_building))) {
			...
			loop index from: 0 to: length(groups) - 1 step: 1 {
				bool is_infectgroup <- false;
				loop indexf from: 0 to: length(groups[index]) - 1 step: 1 {
					if (groups[index][indexf].is_infected) {
						bool is_infectgroup <- true;
						break;
					}
				...
				if (is_infectgroup) {
					loop indexf from: 0 to: length(groups[index]) - 1 step: 1 {
						if (groups[index][indexf].is_susceptible and flip(beta)) {
...
// Aspect
	...
	reflex time_to_study when: !is_dead and !is_hospitalized and juvenile and current_date.hour >= start_work and objective = "resting" {
		objective <- "working";
		target <- any_location_in(school);
	}

	reflex time_to_go_home when: current_date.hour >= end_work and (objective = "working" or senior) {
		objective <- "resting";
		target <- any_location_in(living_place);
	}

	reflex time_to_go_worship when: (senior or believer) and current_date.day_of_week = 7 and current_date.hour >= 10 and objective = "resting" {
		objective <- "resting";
	...
	reflex staying when: target = nil and objective != "working"{
		...
		if flip(staying_counter / staying_coeff) {
			target <- any_location_in(one_of(stuff_place_1, stuff_place_2));
		}
